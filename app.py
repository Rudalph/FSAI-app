...
@app.route('/export_sorted_csv', methods=['POST'])
def export_sorted_csv():
    if request.method == 'POST':
        collection_ref = db.collection('mandalsSelectedForNext')
        csv_filename = 'output_sorted.csv'

        # Retrieve documents sorted by 'finalAuditScore' in descending order
        documents = collection_ref.order_by('finalAuditScore', direction=firestore.Query.DESCENDING).stream()
        ...
        def generate_sorted_csv():
            with open(csv_filename, 'rb') as csvfile:
                data = csvfile.read()
                yield data

        # Send the CSV file as a downloadable attachment
        response = Response(generate_sorted_csv(), content_type='text/csv')
        response.headers["Content-Disposition"] = f"attachment; filename={csv_filename}"
        return response
